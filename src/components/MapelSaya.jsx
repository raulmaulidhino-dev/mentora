    ...
    return (
        <div className="bg-gray-100 flex h-screen">
            ...
            <main className="p-4 basis-[75vw]">
                <h1 className="text-3xl font-extrabold mb-[0.5em]">MAPEL SAYA</h1>
                <section className="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4">
                    {tasks.map((task, index) => {
                        return (<Mapel title={task.title} subject={task.subject} studentClass={task.studentClass} teacherName={task.teacherName} year={task.year} />);
                    })};
                </section>
            </main>
        </div>
